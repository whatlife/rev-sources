...
function jBox(type, options) {
	...
	this._addOverlay = function() {
		...
		if (!this.overlay) {
			...
			this.overlay = jQuery('#jBox-overlay').length ? jQuery('#jBox-overlay').css({zIndex: Math.min(jQuery('#jBox-overlay').css('z-index'), (this.options.zIndex - 1))}) : (jQuery('<div/>', {id: 'jBox-overlay'}).css({display: 'none', zIndex: (this.options.zIndex - 1)}).appendTo(jQuery('body')));
